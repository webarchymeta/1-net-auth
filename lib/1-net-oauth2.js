...
let Authenticator = function () {
        ...
        return (req, res, next) => {
            ...
            userStore.getAccessControl(policyId).then((policy) => {
                if (!req.user) {
                    ...
                    let state = {
                        tid: transId,
                        returnTo: schema + req.headers['host'] + (config.accessPort ? ':' + config.accessPort : '') + req.originalUrl
                    };
                    ...
                    authUrl += '&redirect_uri=' + encodeURIComponent(schema + req.headers['host'] + (config.accessPort ? ':' + config.accessPort : '') + config.returnPath);
    ...
    self.logout = (req, res) => {
        return userStore.unmapUser(req.user).then(() => {
            ...
            logoutUrl += '?returnUrl=' + encodeURIComponent(config.accessProtocol + '://' + req.headers['host'] + (config.accessPort ? ':' + config.accessPort : ''));
            ...
            if (!old_state || old_state.tid !== state.tid) {
                ...
                let callOpts = {
                    ...
                    json: {
                        ...
                        code: req.query.code,
                        redirect_uri: schema + req.headers['host'] + (config.accessPort ? ':' + config.accessPort : '') + config.returnPath,
                        client_id: config.vnet.clientId,
                        client_secret: config.vnet.clientSecret
                    }
                };
